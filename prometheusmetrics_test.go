...
func TestPrometheusRegistration(t *testing.T) {
	...
	pClient := NewProvider(metrics.DefaultRegistry, "test", "subsys", defaultRegistry, 1*time.Second)
...
func TestUpdatePrometheusMetricsOnce(t *testing.T) {
	...
	pClient := NewProvider(metricsRegistry, "test", "subsys", prometheusRegistry, 1*time.Second)
